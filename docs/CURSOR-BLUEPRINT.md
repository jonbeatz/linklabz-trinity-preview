...
- **2026-09-23** — Harvest merged to `main` (`7bb521f`, branch `harvest-godui-keepers`). Jon picked GitHub Pages as the single preview path over Muse artifact links — in-flight artifact republish cancelled (card-concepts artifact still serves pre-harvest). `og:image` was a relative path so unfurlers ignored it (gray "J" in iMessage); fixed to absolute URL (`4617849`, branch `fix-og-image`), verified live. Rollback snapshot `LinkLabz-snapshot-2026-09-23-pre-harvest.zip` (pre-merge `main`) on Drive root. Bridge NEED-ACK sent to Ravyn: final-pass coordination (link inventory, best-of shortlist, blueprint mirror, GitHub plan).
